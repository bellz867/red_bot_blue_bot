...
error_left = np.zeros((1,2))
error_left_derivative = np.zeros((1,3))
error_left_integral = np.zeros((1,5))
error_right = np.zeros((1,2))
error_right_derivative = np.zeros((1,3))
error_right_integral = np.zeros((1,5))
...
def closed_loop_command():
    ...
    previous_mean_right = np.mean(error_right,axis=1)
    previous_mean_left = np.mean(error_left,axis=1)
    ...
    for index_of in xrange(0,error_left.shape[1]-1):
            error_left[0,index_of] = error_left[0,index_of+1]
            error_right[0,index_of] = error_right[0,index_of+1]

    for index_of in xrange(0,error_left_derivative.shape[1]-1):  
            error_left_derivative[0,index_of] = error_left_derivative[0,index_of+1]
            error_right_derivative[0,index_of] = error_right_derivative[0,index_of+1]

    for index_of in xrange(0,error_left_integral.shape[1]-1):
            error_left_integral[0,index_of] = error_left_integral[0,index_of+1]
            error_right_integral[0,index_of] = error_right_integral[0,index_of+1]
    
    # updating the last value for the error
    error_left[0,error_left.shape[1]-1] = left_motor_error
    error_right[0,error_right.shape[1]-1] = right_motor_error

    # updating the last value for the derivative
    error_left_derivative[0,error_left_derivative.shape[1]-1] = (np.mean(error_left,axis=1)-previous_mean_left)/time_diff
    error_right_derivative[0,error_right_derivative.shape[1]-1] = (np.mean(error_right,axis=1)-previous_mean_right)/time_diff

    # updating the last value for the integral
    error_left_integral[0,error_left_integral.shape[1]-1] = np.mean(error_left,axis=1)
    error_right_integral[0,error_right_integral.shape[1]-1] = np.mean(error_right,axis=1)

    # updating the desired left and right commands
    left_motor_effort = KCP*np.mean(error_left,axis=1) + KCD*np.mean(error_left_derivative,axis=1) + KCI*np.mean(np.trapz(error_left_integral,dx=time_diff,axis=1))
...
# function: left_bumper_read
# purpose:
# 1) Read "left_bumper" topic
def left_bumper_read(data):
    global left_bumper_value
    left_bumper_value = data.data
...
def start():
    ...
    left_bumper_sub = rospy.Subscriber("left_bumper", Int16, left_bumper_read)
