...
def right_ir_to_error(data):
    ...
    # taking the inverse
    right_ir_read = 1.0/(-pow(abs(min_distance - right_ir_read - 1),1/(0.5 + random.random())))
...
def start():
    ...
    errorL = rospy.Publisher('right_error', Float32, queue_size=10)
    # starting the node 
    rospy.init_node('right_error_node',anonymous=True)
